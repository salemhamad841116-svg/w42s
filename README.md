...
## 🔬 OTC AI Trading Scanner (رادار التداول الآلي)

A built-in AI-powered radar for OTC (Over-The-Counter) markets, specifically designed for IQ Option-style binary options. The scanner performs candlestick analysis using Google Gemini AI combined with technical indicators (ATR & Bollinger Bands).

### Features
- **4 Auto-Scan Intervals**: 30s, 45s, 1m, 5m with animated countdown ring
- **AI Signal Analysis**: Uses Google Gemini 1.5 Flash for BUY/SELL recommendations
- **Technical Indicators**: ATR (Average True Range) + Bollinger Bands calculated without external libraries
- **Signal History**: Stores last 50 analysis results
- **Rate Limiting**: Built-in cooldown (20s) and mutex lock to prevent concurrent requests
- **Fallback Mode**: Works without Gemini API key using simulated analysis

### Supported OTC Pairs
`EURUSD-OTC` | `GBPUSD-OTC` | `USDJPY-OTC` | `AUDUSD-OTC` | `EURGBP-OTC` | `USDCHF-OTC`

### API Endpoints

| Endpoint | Method | Description |
|----------|--------|-------------|
| `/api/otc/scan` | POST | Run AI analysis scan `{pair: "EURUSD-OTC"}` |
| `/api/otc/pairs` | GET | List supported OTC pairs |
| `/api/otc/history` | GET | Get last 50 signal results |
| `/api/otc/status` | GET | Check analyzer status |

### Configuration (.env)
```env
GEMINI_API_KEY="your_google_ai_key"
IQ_OPTION_EMAIL="demo@example.com"
IQ_OPTION_PASSWORD="demo_password"
IQ_OPTION_MODE="demo"
OTC_SCAN_COOLDOWN_MS="20000"
OTC_GEMINI_MODEL="gemini-1.5-flash"
OTC_MAX_HISTORY="50"
```
...
```json
{
  "signal": "BUY",
  "confidence": "85%",
  "reason": "السعر كسر النطاق السفلي لمؤشر البولينجر باند مع تزايد في قيم ATR"
}
```
