...
export class CamarillaStrategy extends BaseStrategy {
  ...
  public calculate(symbol: string, currentPrice: number): CamarillaCalculatedLevels {
    ...
    const atrValue = defaultAtrPips * this.multiplier;

    const highEdge = Number((currentPrice + atrValue).toFixed(isGoldOrCrypto ? 2 : 5)); // R3
    const lowEdge = Number((currentPrice - atrValue).toFixed(isGoldOrCrypto ? 2 : 5));   // S3
    const pivot = Number(currentPrice.toFixed(isGoldOrCrypto ? 2 : 5));
    const fullRange = Number((highEdge - lowEdge).toFixed(isGoldOrCrypto ? 2 : 5));

    const step0 = Number((highEdge - fullRange * 0.1905).toFixed(isGoldOrCrypto ? 2 : 5)); // R2
    const step1 = Number((highEdge - fullRange * 0.3886).toFixed(isGoldOrCrypto ? 2 : 5)); // R1
    const step2 = Number((highEdge - fullRange * 0.6476).toFixed(isGoldOrCrypto ? 2 : 5)); // S1
    const step3 = Number((highEdge - fullRange * 0.8133).toFixed(isGoldOrCrypto ? 2 : 5)); // S2
